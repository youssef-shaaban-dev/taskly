...
import { TASK_STATUSES, TaskStatus, ProjectTask } from "../types";
...
export const TasksBoard = ({ projectId, onTaskClick, searchQuery = "" }: TasksBoardProps) => {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    // Extract the full task that we injected via DraggableTaskCard
    const taskData = event.active.data.current?.task as ProjectTask;
            ...
            <div className="flex items-center justify-between mt-auto">
              ...
              <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-[9px] font-bold overflow-hidden border-2 border-white shadow-sm" title={activeTask.assignee?.name || "Unassigned"}>
                {activeTask.assignee?.name
                  ? activeTask.assignee.name.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase()
